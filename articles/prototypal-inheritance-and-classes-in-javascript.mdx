...
excerpt: "In practice, we often need to create many similar objects like a list of phones, employees, etc. So, we can achieve this by creating custom types in JS using _Constructor Function_ and then creating multiple objects from it. In other programming languages, we generally use `class` to define this kind of custom type but in JS, the class system is built directly using functions."
---

![grayscale photo of family's hands](/inheritance.jpg)
